        ...
        <div class="chat-container">
            ...
            <div class="chat-area">
                <div id="chatHeader" style="padding: 10px; border-bottom: 1px solid #ccc;">
                    ${selectedUser ? `<h3>Chat with ${selectedUser.username}</h3>` : '<h3>Select a user to chat</h3>'}
                </div>
                <div id="messagesContainer" class="messages-container"></div>
                <form id="messageForm" class="message-input-form" ${!selectedUser ? 'style="display:none;"' : ''}>
                    <input type="text" id="messageInput" class="message-input" placeholder="Type a message..." ${!selectedUser ? 'disabled' : ''}>
                    <button type="submit" class="send-button" ${!selectedUser ? 'disabled' : ''}>Send</button>
                </form>
            </div>
            <div id="connectionStatus" class="connection-status ${isConnected ? 'connected' : 'disconnected'}">
...
async function loadMessages(userId) {
    try {
        const response = await fetch(`messages.php?senderId=${currentUser.id}&receiverId=${userId}`);
        const data = await response.json();
